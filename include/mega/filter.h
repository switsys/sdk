...
class MEGA_API FileAccess;
class MEGA_API InputStreamAccess;
...
protected:
    Filter(const string& text, const bool inheritable, const FilterType type);
...
public:
    ...
    bool match(const string_pair &p, const bool onlyInheritable) const;
...
public:
    ...
    bool excluded(const string_pair& p, const bool onlyInheritable) const;

    // Checks if the name/path pair is to be included.
    bool included(const string_pair& p, const bool onlyInheritable) const;
